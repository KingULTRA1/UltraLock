/* UltraLock🔒 — Single-file, dependency-free address copy/paste integrity protector
...
(function () {
  ...
  async function onCopy(e) {
    try {
      ...
      // Compare with existing metadata (clipboard or memory)
      const existing = await tryReadClipboardMeta();
      if (existing) {
        try {
          if (existing.fingerprint && existing.fingerprint !== fp) {
            // Immediate alert & block copy
            e.preventDefault();
            e.stopImmediatePropagation();
            showBlockingAlert('⚠️ Attention: The address you copied does not match the original verified address. UltraLock enabled. This transaction is NOT secure. Recommendation: Clear your browser or use another device for this transaction.');
            return;
          }
          if (existing.chain && existing.chain !== detected.chain) {
            e.preventDefault();
            e.stopImmediatePropagation();
            showBlockingAlert('⚠️ Attention: The address you copied does not match the original verified address. UltraLock enabled. This transaction is NOT secure. Recommendation: Clear your browser or use another device for this transaction.');
            return;
          ...
          e.preventDefault();
          e.stopImmediatePropagation();
          showBlockingAlert('⚠️ Attention: Clipboard metadata is malformed. UltraLock enabled — copy blocked.');
          return;
        }
      }

      const meta = { address: canonical, chain: detected.chain, fingerprint: fp, ts: Date.now() };
      // Try to write dual clipboard MIME; fallback writes in-memory meta
      await tryWriteDualClipboard(sel, meta);
      // Always write in-memory (helps single-tab use)
      writeMemoryMeta(meta);
      // Visual lock
      showLock(fp);
    } catch (err) {
      console.error('UltraLock copy failed', err);
      try { e.preventDefault(); e.stopImmediatePropagation(); } catch (e) {}
      showBlockingAlert('⚠️ UltraLock encountered an error and blocked the copy to preserve integrity.');
    }
  }
  ...
  async function onPaste(e) {
    try {
      ...
      if (!pasted) return; // nothing to do
      const detected = detectAddress(pasted);
      if (!detected) return; // not an address-like paste — let it through
      const canonical = canonicalize(detected.address, detected.chain);
      const fp = await fingerprintFor(canonical, detected.chain);

      // Check metadata (clipboard or memory)
      const meta = await tryReadClipboardMeta();
      if (!meta || meta.fingerprint !== fp || meta.chain !== detected.chain || (Date.now() - (meta.ts || 0)) > METADATA_TTL_MS) {
        e.preventDefault();
        e.stopImmediatePropagation();
        showBlockingAlert('⛔ Address compromised or altered. Do not proceed.');
        return;
      }

      // Allowed — make sure the canonical address is inserted
      e.preventDefault();
      const active = document.activeElement;
      if (active && (active.tagName === 'INPUT' || active.tagName === 'TEXTAREA')) {
        const s = active.selectionStart || 0;
        const t = active.selectionEnd || active.value.length;
        const v = active.value;
        active.value = v.slice(0, s) + canonical + v.slice(t);
        active.setSelectionRange(s + canonical.length, s + canonical.length);
        active.dispatchEvent(new Event('input', { bubbles: true }));
        // Attach post-paste monitoring
        attachPostPasteMonitor(active, fp);
        showLock(fp);
      } else if (active && active.isContentEditable) {
        const sel = window.getSelection();
        if (sel && sel.rangeCount) {
          sel.deleteFromDocument();
          sel.getRangeAt(0).insertNode(document.createTextNode(canonical));
          sel.collapseToEnd();
          attachPostPasteMonitor(active, fp);
          showLock(fp);
        }
      ...
      console.error('UltraLock paste failed', err);
      try { e.preventDefault(); e.stopImmediatePropagation(); } catch (e) {}
      showBlockingAlert('⛔ UltraLock encountered an error during paste verification and blocked the operation.');
    }
  }

  // ---------- Post-paste mutation monitoring ----------
  const observed = new WeakMap();

  function attachPostPasteMonitor(el, fingerprint) {
    if (!el) return;
    if (observed.has(el)) return;
    const mo = new MutationObserver(async () => {
      // read current content
      const val = el.isContentEditable ? el.innerText : (el.value || el.textContent || '');
      const det = detectAddress(val);
      if (!det) {
        // changed so much it's no longer an address
        disableElement(el, 'Address disappeared or mutated');
        return;
      }
      try {
        const canon = canonicalize(det.address, det.chain);
        const f = await fingerprintFor(canon, det.chain);
        if (f !== fingerprint) disableElement(el, 'Address fingerprint no longer matches — blocked');
      } catch (e) {
        disableElement(el, 'Post-paste verification failed');
      }
    });
  ...
  function disableElement(el, reason) {
    try {
      ...
      console.warn('UltraLock: failed to disable element', e);
    }
  }

  // ---------- Initialization & public helpers ----------
  function init() {
    document.addEventListener('copy', onCopy, true);
    document.addEventListener('paste', onPaste, true);
    // Expose API for debugging and manual use
    window.UltraLock = {
      detectAddress,
      canonicalize,
      fingerprintFor,
      readMeta: readMemoryMeta,
      writeMeta: writeMemoryMeta,
      tryWriteDualClipboard
    };
  }

  // Auto-init when included
  try { init(); } catch (e) { console.error('UltraLock failed to init', e); }
